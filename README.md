...
└── lib/                          # ユーティリティ
    ...
    ├── aladdin-engine.ts         # アラジンエンジン（相場・マッチング・仕様書生成）
    └── export-utils.ts           # エクスポート機能（Markdown/JSON/PDF/共有URL）
```
...
**10万円の付加価値を提供する**、初心者向けAI発注支援ツール。

### コアエンジン (`src/lib/aladdin-engine.ts`)

| 機能 | 説明 |
|------|------|
| **相場見積もり** | 業界×ユースケース×規模の20+パターンの価格データベース |
| **調整係数** | データ状況・システム連携・セキュリティ・IT体制・納期による補正 |
| **ベンダーマッチング** | 業界適合(40点)+技術適合(30点)+価格適合(20点)+信頼性(10点)のスコアリング |
| **仕様書自動生成** | 回答から標準フォーマットの仕様書を自動生成 |
| **類似事例マッチング** | 業界・ユースケース・予算から関連事例をレコメンド |
| **セッション永続化** | localStorage保存（24時間有効）、復元ダイアログ |

### エクスポート機能 (`src/lib/export-utils.ts`)

| 形式 | 説明 |
|------|------|
| **Markdown** | 仕様書形式でダウンロード |
| **JSON** | データバックアップ用 |
| **PDF** | ブラウザ印刷機能で高品質PDF |
| **クリップボード** | ワンクリックでコピー |
| **共有URL** | Base64エンコードで回答を埋め込み |

### UI構成

```
┌─────────────────────────────────────────────────────────────────┐
│ Header: ロゴ / 進捗バー / リセット / ダウンロード▼              │
│                              [Markdown/JSON/PDF/コピー]         │
├──────────────────────────────┬──────────────────────────────────┤
│                              │ 💰 相場見積もり                   │
│   📋 質問フロー               │    想定費用: 300万〜600万円       │
│                              │    信頼度: 75%                   │
│   Q1. 業界選択               │    内訳・調整要因                 │
│   [製造業] [小売] [金融]...   ├──────────────────────────────────┤
│                              │ 🏢 おすすめベンダー (3社)         │
...
|---|------|--------|---------|
| 1 | 業界選択 | 単一選択 | - |
| 2 | ユースケース | 単一選択 | 業界に応じて選択肢変更 |
| 3 | 課題詳細 | テキスト | - |
| 4 | データ状況 | 単一選択 | - |
| 5 | システム連携 | Yes/No | - |
| 6 | 既存システム | 複数選択 | 連携必要時のみ表示 |
| 7 | IT体制 | 単一選択 | - |
| 8 | 予算 | 単一選択 | - |
| 9 | スケジュール | 単一選択 | - |
| 10 | セキュリティ | 複数選択 | - |
...
[完了] Phase 0: モック開発
       ...
       └── レスポンシブ対応
       ↓
[完了] Phase 0.5: AI Studio コアエンジン ★NEW
       ├── アラジンエンジン実装
       │   ├── 相場データベース（20+パターン）
       │   ├── 調整係数による見積もり精度向上
       │   ├── ベンダーマッチング（重み付けスコアリング）
       │   ├── 仕様書自動生成
       │   └── 類似事例マッチング
       ├── エクスポート機能
       │   ├── Markdown/JSON/PDF出力
